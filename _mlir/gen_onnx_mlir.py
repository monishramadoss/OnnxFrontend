...
for schema in defs.get_all_schemas_with_history():
    version_dict[schema.name] += [int(schema.since_version)]       
    version_dict[schema.name].sort(reverse=True)    
...
if __name__ == "__main__":
    ...
    class Args(object):
        ...
        if dry_run:
            ...
            if not os.path.exists(os.path.join(curr_dir,'..', 'out')):
                os.makedirs(os.path.join(curr_dir,'..', 'out'))
                
            op_def_file_path = os.path.join(curr_dir,'..', 'out', "ONNXOps.td.inc")
            op_def = io.open(op_def_file_path, "w", newline="")
            op_importer_file_path = os.path.join(curr_dir,'..', 'out', "OpBuildTable.inc")
